...
export interface BundleReportOptions {
     ...
     * The report will include selected sections from the following:
     * emitted chunks/assets, dependency packages, source modules,
     ...
     * Defaults to all sections (except `entryChunks` which is always included).
     *
     * Available sections: `dependencyPackages`, `sourceModules`, `shipped`, `visualizer`.
...
export function bundleReportPlugin(options: BundleReportOptions): Plugin[] {
    ...
    const reportPlugin: Plugin = {
        ...
        closeBundle() {
            ...
            const visualizerData = visualizerTempFile && fs.existsSync(visualizerTempFile)
    ...
    if (includeVisualizer) {
        plugins.push(visualizer({ filename: visualizerTempFile!, json: true }) as Plugin);
    }

    plugins.push(reportPlugin);
